...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        # 将一个batch的句子所有词的embedding与已构建好的positional embedding 相加
        # (这里按照该批次数据的最大句子长度来取对应需要的那些positional embedding 值)
        # pe：1× max_len× embedding X：1x token_length * d_model
...
x  →  Linear(d_model → d_ff)  →  ReLU  →  Dropout  →  Linear(d_ff → d_model)  →  输出
