...
def print_memory():
    ...
    mps_allocated = torch.mps.current_allocated_memory() / (1024 ** 3) if torch.backends.mps.is_available() else 0
    print(f"RSS: {rss:.2f} GB, MPS allocated: {mps_allocated:.2f} GB")
...
def run_epoch(data, model, loss_compute, is_train=False, use_profiler=False):
    ...
    for step, batch in enumerate(tqdm(data)):  # tqdm用于显示处理进度条
        ...
        # if i > 100 & i < 1000:  # 只打印几个 batch
        #     print(f"数据加载 {i}: {t1 - t0:.3f}s, 转移到设备: {t2 - t1:.3f}s, "
        #           f"前向: {t3 - t2:.3f}s, 损失计算: {t4 - t3:.3f}s, "
        #           f"反向+优化: {t5 - t4:.3f}s")
