...
class RelativeMultiheadAttention(nn.Module):
    ...
    def _in_projection_packed(self, q, k, v):
        ...
        q, k, v = F.linear(q, w, b).chunk(3, dim=-1)  # 修复：使用F.linear
        return q, k, v
...
class RelativeTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, rel_pos_embedding=None, src_mask=None, src_key_padding_mask=None):
        # 自注意力
        src2 = self.norm1(src)
        src2 = src2.permute(1, 0, 2)  # [seq_len, batch_size, embed_dim]
        ...
        src2 = src2.permute(1, 0, 2)  # [batch_size, seq_len, embed_dim]
...
class EnhancedTransformerModel(nn.Module):
    ...
    def forward(self, src, src_mask=None):
        ...
        if self.use_relative_pos:
            ...
            # 转换为非负索引
            relative_position = relative_position + seq_len - 1  # 范围从0到2*seq_len-2

            # 获取相对位置编码
            rel_pos_embedding = self.pos_encoder.rel_pos_bias(relative_position)  # [seq_len, seq_len, embed_size]

            # 扩展以匹配批次大小
            rel_pos_embedding = rel_pos_embedding.unsqueeze(0).expand(batch_size, -1, -1,
                                                                      -1)  # [batch_size, seq_len, seq_len, embed_size]
...
def main():
    ...
    config = {
        ...
        'use_relative_pos': True,  # 是否使用相对位置编码
        'beam_width': 3,  # beam search宽度，大于1时启用beam search
        'min_freq': 2,  # 过滤低频词的最小频率
        'unk_threshold': 0.1,  # 低频词比例阈值
        'unk_penalty': 2.0,  # 生成<unk>的惩罚系数
